...
function showLoadingPackageType() {
    $('#PackageTypeLoadingOverlay').removeClass('d-none').css('display', 'flex');
}

function hideLoadingPackageType() {
    $('#PackageTypeLoadingOverlay').addClass('d-none').css('display', 'none');
}
...
$(document).on('click', 'a[href="#"]', function (e) {
    ...
    if ($(this).text().includes('Khuyến mãi')) {
        ...
        $.get(`/api/v1/promotions`, function (data) {
...
function renderPromotions(data) {
    let html = '';
    if (data.length === 0) {
        html = '<p class="text-muted">Không tìm thấy khuyến mãi nào.</p>';
    } else {
        data.forEach(p => {
            html += `
                <div class="p-3 border rounded mb-2 promo-item cursor" data-id="${p.promoId}"
                data-discount="${p.discountAmount}">
                    <strong>${p.code}</strong> - ${p.description}
                    <br><small>Áp dụng: ${p.durationDays || 0} ngày</small>
                </div>`;
        });
    }
...
function applyPromoDiscount(originalPrice) {
    const discountAmount = parseFloat($('#promotionText').data('discount')) || 0;
    const finalPrice = Math.max(0, originalPrice - discountAmount);
    $('#total-price').text(finalPrice.toLocaleString('vi-VN') + ' đ');
}

$(document).on('click', '.promo-item', function () {
    $('#promotionModal').modal('hide');

    $('.promo-item').removeClass('selected'); 
    $(this).addClass('selected');  

    const promoCode = $(this).find('strong').text().trim();
    const discountAmount = parseFloat($(this).data('discount')) || 0;

    $('#promotionText')
        .html(`<strong>${promoCode}</strong>`)
        .data('discount', discountAmount);

    const originalTotal = parseFloat($('#total-price').data('original')) || 0;
    applyPromoDiscount(originalTotal);
});
...
$('#promotionSearchBtn').on('click', function () {
    ...
    $.get(`/api/v1/promotions?keyword=${encodeURIComponent(keyword)}`, function (data) {
        renderPromotions(data);
    });
});
