async function collectMainFormData() {
    ...
    if (priceInput) {
        ...
        if (rawPrice) {
            data['price'] = parseFloat(rawPrice);
        }
    }
    ...
    const selectedPromotion = document.querySelector('.promo-item.selected');
...
document.getElementById('testCollectFormBtn')?.addEventListener('click', async function () {
    ...
    if (rawData.promotionId) {
        formData.set("promotionId", rawData.promotionId);
    }
    ...
    formData.set("price", rawData.price);
